...
conn.execute('''CREATE TABLE IF NOT EXISTS RS_table
                (issue_key TEXT PRIMARY KEY, heading2 INTEGER, heading3 INTEGER, heading4 INTEGER)''')
conn.execute('''CREATE TABLE IF NOT EXISTS SRS_table
                (issue_key TEXT PRIMARY KEY, heading2 INTEGER, heading3 INTEGER, heading4 INTEGER)''')
        ...
        if issue_content[i]['type'] == 'heading':
            try:
                ...
                if issue_content[i]['attrs']['level'] == 2:
                    heading2_num = int(num.split('.')[1])
                elif issue_content[i]['attrs']['level'] == 3:
                    heading3_num = int(num.split('.')[2])
                elif issue_content[i]['attrs']['level'] == 4:
                    heading4_num = int(num.split('.')[3])
            except IndexError:
                pass
...
ORDER BY heading2,heading3,heading4;"""
...
conn.execute('DELETE FROM RS_table')
conn.execute('DELETE FROM SRS_table')
