...
def repeat_select_delete(stop_event):
    while not stop_event.is_set():
        with Session() as session:
...
def main():
    ...
    t1 = threading.Thread(target=repeat_insert, args=(stop_event,))
    t2 = threading.Thread(target=repeat_select_delete, args=(stop_event,))
    t3 = threading.Thread(target=print_cnt, args=(stop_event,))
